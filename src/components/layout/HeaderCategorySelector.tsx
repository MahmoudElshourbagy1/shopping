...
const HeaderCategorySelector = async() => {
  ...
  return (
    <div className='relative inline-block'>
        <button className='peer group text-gray-700 hover:text-gray-900  cursor-pointer text-sm font-medium flex items-center gap-1 '>
        Categories
            <svg 
            ...
            <div className='w-64 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden'>
                <div className='py-2'>
                    {categories.map((category)=>(
                        ...
                        className='block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 duration-100 transition-all hover:text-gray-900'
                        >
                        {category.title}
                        </Link>
                    ))}
