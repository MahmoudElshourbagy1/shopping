...
const getTextStyle=():React.CSSProperties=>{
    ...
    const y = Math.sin(radian) * radian - 5
    return{
        position: 'absolute',
        width:'200px',
        height:'80px',
...
const WinningItem =({product,onClose}:{product:Product ,onClose:()=>void})=>{
    ...
    return(
        <div className='text-center animate-[slideUp_0.5s_ease-out]'>
            <div className={`p-8 rounded-xl bg-white shadow-2xl backdrop-blur-lg bg-opacity-90 border border-white/20 transform transition-all duration-500 hover:shadow-emerald-500/20 hover:scale-[1.01]`}>
            <div className='relative p-4'>
                <div className='absolute inset-0 bg-gradient-to-r from-emerald-500/10 to-sky-500/10 animate-pulse rounded-lg'/>
                <h3 className={`text-2xl font-bold text-emerald-600 p-4 mb-8 animate-[pulse_2s_ease-in-out_infinite] [text-shadow:_0_1px_2px_rgb(0_0_0/_10%)]`
                }>
                🎉 Congratulations 🎉
                </h3>
                <div className='flex flex-col items-center gap-6'>
                    {product.image && (
                        <div className='relative group'>
                            <div className={`absolute -inset-4 bg-gradient-to-r from-yellow-400 via-red-500 to-pink-500 rounded-2xl opacity-75 blur-lg animate-pluse group-hover:opacity-100 transition-opacity duration-500`}/>
                            <div className='relative bg-gradient-to-br from-white to-gray-50 p-4 rounded-xl shadow-2xl'>
                                <div className='absolute -top-3 -right-3 bg-red-600 text-white px-4 py-1 rounded-full font-black text-lg shadow-xl z-10'>
                                    FREE!
                                </div>
                ...
                disabled={isAdding}
                className={`
                    mt-6 w-full py-4 px-8 rounded-full font-bold transition-all duration-300 transform 
                    flex items-center justify-center gap-2 hover:scale-105 active:scale-95 disabled:opacity-50
                    disabled:cursor-not-allowed cursor-pointer bg-gradient-to-r from-emerald-400 to-emerald-600 text-white
                    `}
                >
                    {isAdding ? (
                        <>
...
const WheelOfFortune = ({ products, winingIndex }: WheelOfFortuneProps) => {
    const [isOpen,setIsOpen]=useState<boolean>(false)
    const [showWiningItem,setShowWiningItem]= useState<boolean>(false)
  ...
  <DialogContent className='sm:max-w-[800px] p-0'>
   <DialogTitle>
    <div className='p-6 text-center relative overflow-hidden'>
        <div className='absolute inset-0 bg-gradient-to-r  to-orange-500/20 from-red-500/20 animate-pulse'/>
   ...
   <div className='flex flex-col items-center justify-center p-8 gap-4 bg-gray-50'>
    <div
     ...
     ${showWiningItem ? 'scale-0 opacity-0 rotate-100':'scale-100 opacity-100'}
     `}>
        {/* Red pointer */}
        <div 
        className={`
    ...
    className={`
        ...
        ${!showWiningItem ? 'scale-0 opacity-0 translate-y-full':'scale-100 opacity-100 transition-y-0'}
        `}
    >
        {hasSpun && !isSpinning &&(
            <WinningItem 
            product={products[winingIndex]}
    ...
    className={`
        ...
        ${showWiningItem ? 'opacity-0 scale-0 -translate-y-full':''}
    ...
    >
        {isSpinning ? (
            ...
            <>
            <span className='animate-[pluse_1s_ease-in-out_infinite]'>🎁</span>
            {" SPIN NOW! "}
            <span className='animate-[pluse_1s_ease-in-out_infinite]'>🎁</span>
            </>
        )}
